...
const getUserPosts = (req, res, next) => {
  ...
  db.any(
    "SELECT posts.id AS postID, posts.post_body, users.id AS userID, posttype, posts.img_url FROM posts JOIN users on(users.id = posts.user_id) WHERE posts.user_id=$1",
    [req.body.user_id]
  )
    .then(posts => {
      res.status(200).json({
