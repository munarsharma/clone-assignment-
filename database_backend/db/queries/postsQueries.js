...
const getUserPosts = (req, res, next) => {
  ...
  db.any(
    "SELECT users.id AS user_id, users.img_url AS user_img, posts.id AS id, username, posttype, post_body, posts.img_url AS img_url, COUNT(DISTINCT likes.id) AS all_likes FROM posts RIGHT JOIN users ON posts.user_id = users.id LEFT JOIN likes ON posts.id = likes.post_id WHERE posts.user_id=$1 GROUP BY users.id, posts.id ORDER BY posts.id DESC",
    [user_id]
  )
    .then(posts => {
      res.status(200).json({
